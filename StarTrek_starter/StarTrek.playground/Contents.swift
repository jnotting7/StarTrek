...
let numHumans = humanOfficers.count
print(numHumans)



// Q1: What percentage of humans are female?
let femaleHumans = humanOfficers.filter { femaleOfficers.contains($0) }.count
print(femaleHumans)
let percentHumanFemales = Float(femaleHumans)/Float(numHumans)
print(percentHumanFemales)



// Q2: How many first officers are human?
let officerNames = firstOfficers.map{$0.key}.sorted()
let humanFirstOfficers = officerNames.filter{humanOfficers.contains($0)}.count
...
print(allCaptains.map{$0.name}.sorted())

/*:
Group 2
*/
// Q6: Create an array of officers serving as first officers who are also Vulcan
let vulcanOff: [Officer] = firstOfficers.map{ Officer(name: $0, ship: $1)}
let justOff = vulcanOff.map{$0.name}
let vulc = justOff.filter{ vulcanOfficers.contains($0) }
print(vulc)
...
let numVulcOff = vulc.reduce(0) { $1.count < 5 ? $0 + 1 : $0 }
print(numVulcOff)
